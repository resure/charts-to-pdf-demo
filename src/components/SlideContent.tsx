...
export const SlideContent: React.FC<SlideContentProps> = ({ data }) => {
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <h1 style={{ margin: 0, marginBottom: "8px", fontSize: "28px", color: "#363636" }}>
        {data.title}
      </h1>
      {data.subtitle && (
        <p style={{ margin: 0, marginBottom: "12px", fontSize: "14px", color: "#666666" }}>
          {data.subtitle}
        </p>
      )}

      {data.description && (
        <p style={{ margin: 0, marginBottom: "16px", fontSize: "13px", color: "#444444", lineHeight: 1.5 }}>
          {data.description}
        </p>
      )}
